...
void PID::UpdateError(double cte) {
    ...
    if (yes_i_wanna_twiddle && step % (n_settle_steps + n_eval_steps) == 0){
        ...
        if (!tried_adding && !tried_subtracting) {
            ...
            IndexToParameterAddition(param_index, dp[param_index]);
            tried_adding = true;
        }
        else if (tried_adding && !tried_subtracting) {
            // try subtracting dp[i] from params[i]
            IndexToParameterAddition(param_index, -2 * dp[param_index]);     
...
void PID::IndexToParameterAddition(int idx, double amount) {
    if (idx == 0) {
        Kp += amount;
    }
    else if (idx == 1) {
